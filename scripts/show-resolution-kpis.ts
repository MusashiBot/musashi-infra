...
const resolutionsLast24h = (recentRunsResult.data ?? []).reduce(
  (sum, row) => sum + (Number(row.resolutions_detected) || 0),
  0
);

const kpis = {
  settlement_ready_unresolved: settlementReadyResult.count,
  closed_waiting_settlement: closedWaitingResult.count,
  missing_settles_at: missingSettlesAtResult.count,
  source_missing_unresolved: sourceMissingResult.count,
